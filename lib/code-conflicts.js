...
function validateClaimLineFormat(rawInput, parsedCodes) {
  ...
  parsedCodes.forEach(code => {
    ...
    const xRequiredCategories = ['M45', 'T08', 'T10', 'T12', 'V98', 'V99'];
    ...
    if (baseCode.startsWith('M653')) {
      if (baseCode.length === 5 && baseCode[4] !== '4') {
        findings.push({
          type: 'SPECIFICITY_ERROR',
          code: clean,
          message: `Inappropriate 5th character for trigger finger (${clean}). The 5th character must be 4 (Hand), i.e. M65.34.`
        });
      }
    }
    if (baseCode === 'M7156') {
